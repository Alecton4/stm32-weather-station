...
static struct sFONT *LCD_Currentfonts = &Font8x16; //英文字体
...
static void ILI9341_GPIO_Config(void)
{
	...
	GPIO_Initure.Pin = GPIO_PIN_7 | GPIO_PIN_8 | GPIO_PIN_9 | GPIO_PIN_10 | GPIO_PIN_11 | GPIO_PIN_12 | GPIO_PIN_13 |
...
static void ILI9341_FSMC_Config(void)
{
	...
	SRAM_Handler.Init.WaitSignalActive =
		FSMC_WAIT_TIMING_BEFORE_WS; //存储器是在等待周期之前的一个时钟周期还是等待周期期间使能NWAIT
...
/**
  ...
  * @retval None
  */
void LCD_SetFont(struct sFONT *fonts)
{
...
{
	ILI9341_Clear(0, Line, LCD_X_LENGTH, ((struct sFONT *)LCD_GetFont())->Height); /* 清屏，显示全黑 */
}
