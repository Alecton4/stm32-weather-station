	...
	if (ptr->touch_flag == 0) {
		...
		*中文字体大小是16*16的，需要其它字体请自行制作字模*/
		...
		ILI9341_DispString_EN(
			ptr->start_x + (ptr->end_x - ptr->start_x - 16 * 2) / 2,
			//																ptr->start_y+15,
			ptr->start_y + ((ptr->end_y - ptr->start_y - 16) / 2), "CLR");
	} else /*按键按下*/
	{
		LCD_SetColors(CL_WHITE, CL_WHITE);
		ILI9341_DrawRectangle(ptr->start_x, ptr->start_y, ptr->end_x - ptr->start_x, ptr->end_y - ptr->start_y, 1);
...
static void Draw_Shape_Button(void *btn)
{
	...
	case LINE_SINGLE_PIXCEL:
		...
		LCD_SetColors(CL_BLACK, CL_WHITE);
		ILI9341_DrawLine(ptr->start_x + 20, ptr->start_y + (ptr->end_y - ptr->start_y) / 2, ptr->end_x - 20,
	...
	case LINE_2_PIXCEL:
		...
		LCD_DrawUniLineCircle(ptr->start_x + 20, ptr->start_y + (ptr->end_y - ptr->start_y) / 2, ptr->end_x - 20,
	...
	case LINE_20_PIXCEL:
		...
		LCD_DrawUniLineCircle(ptr->start_x + 20, ptr->start_y + (ptr->end_y - ptr->start_y) / 2, ptr->end_x - 20,
				      ptr->start_y + (ptr->end_y - ptr->start_y) / 2, 10);

		break;

	case LINE_WITH_CIRCLE:
		LCD_SetColors(CL_BLACK, CL_WHITE);
		ILI9341_DrawLine(ptr->start_x + 5, ptr->start_y + (ptr->end_y - ptr->start_y) / 2, ptr->end_x - 5,
				 ptr->start_y + (ptr->end_y - ptr->start_y) / 2);

		for (i = 0; i < ((ptr->end_x - ptr->start_x - 10) / 10); i++) {
			ILI9341_DrawCircle(ptr->start_x + 5 + i * 10, ptr->start_y + (ptr->end_y - ptr->start_y) / 2, 3, 1);
		}
	...
	case RUBBER:
		...
		ILI9341_DrawRectangle(ptr->start_x + ((ptr->end_x - ptr->start_x - 24) / 2),
				      ptr->start_y + ((ptr->end_y - ptr->start_y - 24 - 16) / 2), 24, 24, 1);
		...
		*中文字体大小是24*24的，需要其它字体请自行制作字模*/
		...
		ILI9341_DispString_EN(ptr->start_x + (ptr->end_x - ptr->start_x - 16 * 2) / 2,
				      ptr->start_y + ((ptr->end_y - ptr->start_y - 24 - 16) / 2) + 24, "ERS");
...
{
	LCD_SetColors(CL_WHITE, CL_WHITE);
	ILI9341_DrawRectangle(PALETTE_START_X, PALETTE_START_Y, PALETTE_END_X - (PALETTE_START_X + 1),
